...
import {
  ...
  AdminCreateFleetownerTicketDto,
  AdminUpdateFleetownerTicketDto,
} from "./dto/fleetowner-ticket-http.dto";
...
@ApiTags("admin-fleetowner-tickets")
@ApiBearerAuth("JWT-auth")
@UseGuards(JwtAuthGuard)
// @Controller("admin/fleetowner/tickets")
@Controller(["admin/fleetowner/tickets", "web-admin/fleetowner/tickets"])
export class FleetownerTicketHttpController {
  constructor(private readonly service: FleetownerTicketHttpService) {}

  @Post("admin-create")
  @HttpCode(HttpStatus.CREATED)
  @ApiConsumes("multipart/form-data")
  @ApiOperation({ summary: "Admin - Create fleetowner ticket" })
  @ApiBody({
    schema: {
      type: "object",
      required: ["subject"],
  ...
  @ApiConsumes("multipart/form-data")
  @ApiOperation({ summary: "Admin - Update fleetowner ticket" })
  ...
  @Get()
  @ApiOperation({ summary: "Admin - Get all Fleetowner tickets" })
  @ApiResponse({ status: 200, description: "List of fleetowner tickets" })
  findAll(@Query() query: AdminTicketQueryDto) {
    return this.service.getAllTickets(query);
  }

  @Get("pagination")
  @ApiOperation({ summary: "Admin - Fleetowner tickets paginated" })
  @ApiResponse({ status: 200, description: "Paginated fleetowner tickets" })
  ...
  @Get("cursor")
  @ApiOperation({
    summary: "Admin - List fleetowner tickets (cursor pagination)",
    description:
      "Keyset-paginated ticket list. Pass meta.nextCursor as cursor for the next page.",
  })
  @ApiResponse({ status: 200, description: "Cursor-paginated fleetowner tickets" })
  findCursor(@Query() query: AdminTicketCursorDto) {
    return this.service.getTicketsCursor(query);
  }

  @Get(":id")
  @ApiOperation({ summary: "Admin - Get Fleetowner ticket by ID" })
  @ApiParam({ name: "id", description: "Ticket UUID" })
  ...
  @Patch(":id/status")
  @ApiOperation({ summary: "Admin - Change Fleetowner ticket status" })
  @ApiParam({ name: "id", description: "Ticket UUID" })
  ...
  @Patch(":id/assign")
  @ApiOperation({ summary: "Admin - Assign staff to Fleetowner ticket" })
  @ApiParam({ name: "id", description: "Ticket UUID" })
  ...
  @ApiOperation({ summary: "Admin - Get Fleetowner ticket conversations" })
  @ApiParam({ name: "id", description: "Ticket UUID" })
  ...
  @HttpCode(HttpStatus.CREATED)
  @ApiConsumes("multipart/form-data")
  @ApiOperation({ summary: "Admin - Reply to Fleetowner ticket" })
  @ApiParam({ name: "id", description: "Ticket UUID" })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @HttpCode(HttpStatus.CREATED)
  @ApiConsumes("multipart/form-data")
  @ApiOperation({ summary: "Staff - Reply to Fleetowner ticket" })
  @ApiParam({ name: "id", description: "Ticket UUID" })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @Delete(":id")
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: "Admin - Delete Fleetowner ticket" })
  @ApiParam({ name: "id", description: "Ticket UUID" })
