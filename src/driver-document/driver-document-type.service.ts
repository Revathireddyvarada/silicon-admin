...
@Injectable()
export class DriverDocumentTypeService {
  constructor(
    @InjectRepository(DriverDocumentType)
    private readonly repo: Repository<DriverDocumentType>,
  ) {}

  async create(
    dto: CreateDriverDocumentTypeDto,
    ...
    const childRows: Partial<DriverDocumentType>[] = [];
  ...
  async findAll(): Promise<DriverDocumentType[]> {
  ...
  async findAllGrouped(): Promise<
    (DriverDocumentType & { children: DriverDocumentType[] })[]
  > {
    ...
    const childrenByParentId = new Map<string, DriverDocumentType[]>();
    for (const r of all) {
  ...
  async findOne(id: string): Promise<DriverDocumentType> {
    const entity = await this.repo.findOne({ where: { id, isDeleted: false } });
    if (!entity)
      throw new NotFoundException(`Driver document type ${id} not found`);
    return entity;
  }

  async update(
    id: string,
    dto: UpdateDriverDocumentTypeDto,
    currentUserId?: string,
  ): Promise<DriverDocumentType[]> {
    const entity = await this.repo.findOne({ where: { id, isDeleted: false } });
    if (!entity)
    ...
    const results: DriverDocumentType[] = [savedParent];

    const upsertChild = async (
      existing: DriverDocumentType | undefined,
      want: boolean,
      name: string,
      flags: Partial<DriverDocumentType>,
    ) => {
      if (want && !existing) {
  ...
  return { message: "Driver document type permanently deleted" };
}
}
